...
<body>
    ...
    <%
    	...
    	try {
            ...
            String sql = "SELECT * FROM items LEFT JOIN item_photos ON item_photos.item_id = items.item_id WHERE posted_by = ? AND items.is_active = 1 AND item_type = 'Lost' GROUP BY items.item_id";
            PreparedStatement pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, session.getAttribute("loggedInUserId").toString());
    ...
    <%
    	...
    	try {
    		Connection conn = DBUtil.getConnection();
            String sql = "SELECT * FROM items LEFT JOIN item_photos ON item_photos.item_id = items.item_id WHERE posted_by = ? AND items.is_active = 1 AND item_type = 'Found' GROUP BY items.item_id";
            PreparedStatement pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, session.getAttribute("loggedInUserId").toString());
            ResultSet rs = pstmt.executeQuery();
